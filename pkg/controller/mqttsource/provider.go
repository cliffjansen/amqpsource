...
const (
	...
	// mqttSourceEnabledEnvVar is used to determine if the MQTT Source's controller should run.
	// It will only run if the environment variable is defined and has the value 'true'.
	mqttSourceEnabledEnvVar = "ENABLE_MQTT_SOURCE"

	// raImageEnvVar is the name of the environment variable that contains the receive adapter's
	// image. It must be defined.
	raImageEnvVar = "MQTT_RA_IMAGE"
)

// Add creates a new MqttSource Controller and adds it to the Manager with
// default RBAC. The Manager will set fields on the Controller and Start it when
// the Manager is Started.
func Add(mgr manager.Manager) error {
	if enabled, defined := os.LookupEnv(mqttSourceEnabledEnvVar); !defined || enabled != "true" {
		log.Println("Skipping the MQTT Source controller.")
		return nil
	}
